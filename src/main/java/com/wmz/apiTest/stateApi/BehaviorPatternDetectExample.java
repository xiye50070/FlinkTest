...
//        new MapStateDescriptor<Void,Pattern>("pattern", Void.class,Pattern.class);

        BroadcastStream<Pattern> broadcastStream = patternStream.broadcast(descriptor);


        // 连接两条流进行处理
        SingleOutputStreamOperator<Tuple2<String,Pattern>> matches = actionStream.keyBy(Action::getUserId)
